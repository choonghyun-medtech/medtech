...
- HS코드 매핑: 관세청/K-stat이 카테고리명으로 직접 분류를 제공하지 않아, 이 카테고리들을
  ...
  일부 섞여 잡힐 수 있는 넓은 코드라 진폭이 과장될 수 있다는 점에 유의.
    ...
    · 체외진단 PCR : 3822192020 (2026-08-21 신규 추가 — index.html 스펙. 2026-09-08:
      추적 중인 PCR 진단시약 전문기업 씨젠으로 companies 채움)
    · 면역진단 : 3822191000 (2026-08-21 신규 추가 — index.html 스펙. 2026-09-08:
      추적 중인 면역진단(POCT) 전문기업 바디텍메드로 companies 채움)
...
CATEGORIES = [
    ...
    },
    {
        ...
        # 사용자가 지정한 값 그대로 사용. companies는 2026-09-08에 PCR 진단시약 전문
        # 기업인 씨젠(096530)으로 채움(추적 유니버스 내 IVD 종목 중 가장 명확한 매칭).
        "key": "pcr_diagnostic",
        "label": "체외진단 PCR",
        "hsCodes": ["3822192020"],
        "companies": "씨젠",
    ...
    {
        # companies는 2026-09-08에 면역진단(POCT) 전문 기업인 바디텍메드(206640)로 채움.
    ...
    },
    {
        ...
        # 확인하지 못했다. 그런데 이 파일의 pcr_diagnostic(씨젠, 3822192020)과
        # immuno_diagnostic(바디텍메드, 3822191000)이 이미 3822.19 아래 10자리 코드를 쓰고
        # 있어, 이 카테고리의 "382219"(6자리) 전국 총계에는 씨젠·바디텍메드의 진단시약
        # 수출액도 섞여 들어올 수 있다 — device_medical/surgical_device가 HS 901890을
