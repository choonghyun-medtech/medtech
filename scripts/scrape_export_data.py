...
- HS코드 매핑: 관세청/K-stat이 카테고리명으로 직접 분류를 제공하지 않아, 이 카테고리들을
  ...
  일부 섞여 잡힐 수 있는 넓은 코드라 진폭이 과장될 수 있다는 점에 유의.
    · 톡신(보툴리눔) : 2026-09-02 재확인 — HS 개정(2022-01-01 시행)으로 코드가 바뀜.
      ...
      기간별로 이어붙여야 한다고 정정)
    · 미용기기(의료용) : 901890 (6자리 — 클래시스/원텍 등. 루트로닉/레이저옵텍도
      같은 코드를 쓰지만 tickers.json 추적 유니버스 밖이라 companies엔 제외)
    · 임플란트(치과) : 902129 (6자리, 2026-08-21 확정 — 이전 10자리 9021290000에서
      상위 6자리로 넓힘. 덴티움/디오 — 오스템임플란트도 같은 코드를 쓰지만 추적
      유니버스 밖. 정형외과용 인공관절(9021.31)과는 다른 코드이니 혼동 주의)
    · 필러·리쥬란류(기타화장품) : 3304999000 (휴젤 필러, 파마리서치 리쥬란. 휴메딕스도
      같은 코드를 쓰지만 추적 유니버스 밖)
    · 지혈제(지혈재) : 3006104000 (2026-08-21 추가 — 넥스트바이오메디컬 내시경용 지혈재)
    · 치과영상장비 : 902213 (6자리, 2026-08-21 확정 — 이전 10자리 9022120000에서
      상위 6자리로 넓힘. 2026-09-08: 원래 출처가 명시한 제노레이는 추적 유니버스 밖으로
      확인돼 companies에서 제외, 대신 추적 중인 바텍으로 교체 — CBCT 글로벌 1위·수출
      비중 80~90%까지 확인함. 제노레이가 실제로 이 코드에 기여하는 비중은 불명)
    · 체외진단 : 382219(6자리, 3822.19) — 원래 "체외진단 PCR"(3822192020, 씨젠)과
      "면역진단"(3822191000, 바디텍메드) 둘로 나눠 2026-08-21 신규 추가했다가,
      2026-09-16 사용자 요청으로 하나의 "체외진단" 카테고리(HS 3822.19, 지역 서울
      송파구+경기 수원시, companies 씨젠·바디텍메드·에스디바이오센서)로 통합함.
...
CATEGORIES = [
    {
        ...
        # 2026-09-08: tickers.json(국내 추적 39개사) 대비 재검토 — 대웅제약/휴온스글로벌은
        # 추적 유니버스에 없어 제외.
        "companies": "휴젤·메디톡스",
    ...
    },
    {
        ...
        "label": "미용기기(의료용)",
        "hsCodes": ["901890"],
        # 2026-09-08: 루트로닉/레이저옵텍은 tickers.json 추적 유니버스에 없어 제외.
        "companies": "클래시스·원텍",
    ...
    },
    {
        ...
        "hsCodes": ["902129"],
        # 2026-09-08: 오스템임플란트는 tickers.json 추적 유니버스에 없어 제외. 디오(039840)는
        # tickers.json엔 있었지만 이 companies 필드에 빠져 있던 걸 확인해 추가(국내 임플란트
        # 제조사, 70여개국 수출 — DIOnavi 등).
        "companies": "덴티움·디오",
    ...
    },
    {
        ...
        "label": "필러·리쥬란류(기타화장품)",
        "hsCodes": ["3304999000"],
        # 2026-09-08: 휴메딕스는 tickers.json 추적 유니버스에 없어 제외.
        "companies": "휴젤·파마리서치",
        "countries": [],
    ...
    },
    {
        ...
        # 변경(2026-08-21 사용자 확인). 2026-09-08: 제노레이가 tickers.json 추적
        # 유니버스에 없는 것으로 확인돼 companies에서 빠짐 — 대신 추적 중인 바텍(043150,
        # 치과용 CBCT 글로벌 점유율 1위, 매출 80~90%가 수출)으로 교체.
        "key": "dental_imaging",
        "label": "치과영상장비",
        "hsCodes": ["902213"],
        "companies": "바텍",
    ...
    {
        # 2026-08-21 "체외진단 PCR"(HS 3822192020, 씨젠)과 "면역진단"(HS 3822191000,
        # 바디텍메드)으로 따로 있던 걸 2026-09-16 사용자 요청으로 "체외진단" 하나로
        # 통합 — HS코드도 사용자 지정대로 6자리 3822.19(두 10자리 서브코드의 상위
        # 코드)로 단일화하고, 지역은 두 옛 카테고리의 지역(서울 송파구·경기 수원시)을
        # 그대로 합쳐서 둘 다 수집한다.
        # ⚠️ 3822.19는 glucose_monitoring(아이센스 혈당시험지, 2022-01~ 동일하게
        # 3822.19 사용)과 6자리 레벨에서 완전히 같은 코드다 — 이 카테고리의 전국/국가/
        # 지역 수출액에는 혈당시험지 수출분도 섞여 들어올 수 있고, 반대로
        # glucose_monitoring 쪽에도 체외진단(PCR·면역진단) 수출분이 섞인다. 10자리
        # HSK 서브코드로 정확히 나눌 방법을 찾기 전까지는 감안하고 볼 것.
        "key": "ivd_diagnostic",
        "label": "체외진단",
        "hsCodes": ["382219"],
        "companies": "씨젠·바디텍메드·에스디바이오센서",
        "countries": [],
        "regions": ["서울 송파구", "경기 수원시"],
    },
    {
    ...
    },
    {
        ...
        # 지역은 최초 "경기 평택시"(GMP 공장 소재지)로 넣었으나, 실제 첫 수집 결과 평택시는
        # 거의 0에 가까운 실적만 잡히고(사용자가 관세청 사이트로 직접 대조 확인), 관세청
        # 시군구별 수출 신고 데이터는 "경기 성남시"(분당구 사업장 주소) 기준으로 정상 규모
        # 시계열이 들어오는 것으로 확인돼 성남시로 교체함 — 공장 소재지 ≠ 실제 수출 신고지인
        # 사례.
        "key": "ecg_diagnostic",
        "label": "심전계",
        "hsCodes": ["9018111000"],
    ...
    {
        # 2026-09-14 신규 추가 — 아이센스(혈당측정기+혈당시험지). 사용자가 WCO 공식
        # HS2022 연계표(concordance table)로 직접 확인해 알려준 개정 내역:
        #   · 혈당측정기: 9027.80(~2021) → 9027.89(2022~, 미국 관세당국 판례에서도
        #     Accu-Chek 혈당측정 시스템이 9027.89.4530으로 분류되는 것으로 교차 확인)
        #   · 혈당시험지: 3822.00(~2021) → 3822.19(2022~, "기타" 진단용 시약)
        # 톡신과 동일한 hsCodesByPeriod 패턴이되, 톡신은 "같은 품목의 개정 전후 코드"만
        # 다뤘던 것과 달리 이번엔 "디바이스+시험지 두 품목을 한 기간에 합산"하는 것도 함께
        # 처리한다 — resolve_hs_segments가 세그먼트당 hsCodes 리스트를 그대로 합산 조회하므로
        # 별도 로직 변경 없이 그대로 지원됨.
        #
        # ⚠️ 정밀도 한계(2026-09-14 사용자 확인 후 6자리로 진행하기로 결정): 3822.19는
        # 6자리 레벨까지만 확인됐고, 아이센스 혈당시험지 전용 10자리 HSK 서브코드는
        # 확인하지 못했다. 그런데 이 파일의 ivd_diagnostic(체외진단, 씨젠·바디텍메드·에스디바이오센서,
        # 2026-09-16 통합)도 6자리 3822.19를 그대로 쓰고 있어, 이 카테고리의
        # "382219"(6자리) 전국 총계에는 씨젠·바디텍메드·에스디바이오센서의 진단시약 수출액도 섞여 들어올
        # 수 있다 — device_medical/surgical_device가 HS 901890을 공유하는 것과 같은
        # 유형의 한계. 정확한 10자리 코드를 확인하게 되면 hsCodesByPeriod의 "382219"를
        # 그 값으로 좁혀야 한다.
        ...
        "hsCodesByPeriod": [
            ...
            {"hsCodes": ["902789", "382219"], "from": "202201"},
        ],
        "hsCodeNote": "2022-01 HS 개정으로 혈당측정기 9027.80→9027.89, 혈당시험지 3822.00→3822.19로 코드 변경 — 구간별 합산(3822.19는 6자리 레벨이라 다른 진단시약 수출액과 섞일 수 있음)",
        "companies": "아이센스",
        # 강원 원주시=혈당측정기·시험지 생산(원주공장, 연 10억개 스트립 양산), 인천
        # 연수구=CGM(연속혈당측정기) 생산(송도공장) — 2026-09-14 사용자 확인.
        "countries": [("US", "미국"), ("CN", "중국"), ("JP", "일본")],
        "europeCountries": [("DE", "독일"), ("IT", "이탈리아"), ("GB", "영국")],
        "regions": ["강원 원주시", "인천 연수구"],
    },
]
