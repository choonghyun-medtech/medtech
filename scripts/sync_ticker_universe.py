...
- "섹터" 필드는 기존 8개 카테고리(의료기기/로보틱스/미용/치과/체외진단/디지털헬스/생명공학/
  의료서비스 — 전부 한글, 2026-09-17에 IVD/MedTech/Healthcare Provider 3개를 한글로
  통일)로 매핑한다 — 이 값을 수급 판독기 섹터 필터, 기업 스냅샷 드롭다운, 리서치 보고서
  탭 카테고리 등 다른 화면에서 그대로 참조하고 있어서, 대시보드의 38개 세분 섹터를 그대로
  쓰면 그 화면들이 깨진다. 한 기업이 여러 세분 섹터를 가지면 SECTOR_PRIORITY 순서대로 첫
...
# 카테고리 라벨은 전부 한글로 통일한다(2026-09-17 요청) — 예전엔 IVD/MedTech/
# Healthcare Provider 3개가 영어로 남아있어 미용/디지털헬스 등 한글 라벨과 섞여 있었다.
# 매핑 로직(어느 세분 섹터가 어느 카테고리로 가는지) 자체는 그대로, 출력 문자열만 바꿨다.
SECTOR_PRIORITY = [
    ('체외진단', '체외진단'),
    ('치과', '치과'),
    ('메디컬 에스테틱', '미용'),
    ('뷰티', '미용'),
    # 로보틱스/수술로봇보다 구체적인 의료기기 세부군을 먼저 검사한다 — 존슨앤드존슨처럼
...
def map_sector(sectors):
    for tag, category in SECTOR_PRIORITY:
        if tag in sectors:
            return category
    return '의료기기'  # 매칭 안 되는 새 섹터가 생기면 우선 의료기기로(수동 재검토 필요)
