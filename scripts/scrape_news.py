...
- 회사명 -> 카테고리 매핑은 NEWS_COMPANY_CATEGORY에서 관리. 뉴스클리핑_가이드라인.md
  (2026-08-18 사용자 제공 최신본)의 10개 카테고리 틀을 기반으로 하되(2026-09-17: 카테고리
  라벨을 index.html/scrape_news_global.py와 맞춰 전부 한글로 통일하고, tickers.json에 있는
  실제 메드텍 기업은 주가 Performance/기업 스냅샷과 같은 마스터 섹터로 재배치 —
  "Bio-Processing"이 "생명공학"에 흡수되며 11개로 늘었다):
  디지털헬스 / 미용 / 로보틱스 / 휴머노이드 / 산업용·서비스 로봇 /
  로보틱스 밸류체인 / 치료제 / 체외진단 / 생명공학 / 의료기기 / 치과.
...
# 2026-09-17: tickers.json에 있는 실제 메드텍 기업은 주가 Performance/기업 스냅샷과 같은
# 마스터 섹터로 맞췄다(예: 넥스트바이오메디컬 치료제→의료기기, 아이센스 디지털헬스→의료기기).
# 휴머노이드/산업용·서비스 로봇/로보틱스 밸류체인(tickers.json에 없는 로보틱스 비교군)과
# tickers.json에 없는 소형/비상장 회사는 맞춰볼 마스터 기준이 없어 기존 카테고리 유지.
NEWS_COMPANY_CATEGORY = {
    ...
    "인바디": "디지털헬스",
    "뉴로핏": "디지털헬스",
    "GC메디아이": "디지털헬스",  # 구 유비케어, COMPANY_SEARCH_ALIASES에서 옛 이름도 함께 검색
    "에어스메디컬": "디지털헬스",
    "스카이랩스": "디지털헬스",
    "카카오헬스케어": "디지털헬스",
    # 2. 미용
...
CONTEXT_REQUIRED = {
    ...
    "삼성전자": ["로봇", "휴머노이드"],
    "HL홀딩스": ["로봇"],
    # "고영"은 SMT 검사장비 회사명이면서 흔한 한국인 이름이라, 연예 기사에 동명이인으로 자주
    # 오매칭됐다(2026-08-28, 배우 남윤수 앨범 소식 기사에 "고영"이 우연히 매칭된 사례로 확인).
    "고영": ["로봇", "SMT", "검사장비", "코스닥", "공시", "실적", "주가", "수주", "계약", "반도체", "자동화"],
    # "인바디"는 체성분 분석기 회사명이면서 "인바디로 재봤더니 체지방률 OO%"처럼 일반명사화돼
    # 다이어트/몸매 연예 기사에 그냥 측정 도구 이름으로 스쳐 지나가듯 언급되는 경우가 많다
    # (2026-09-03, 연예인 다이어트 기사에 "인바디"만 우연히 언급된 사례로 확인 — 기사 자체에
    # 회사 관련 실질 내용이 전혀 없었음). "체성분"/"체지방" 같은 측정 관련 단어는 일반 기사에도
    # 흔히 같이 나와 안전장치가 안 되므로 뺐고, 실제 비즈니스 맥락 단어만 요구한다.
    "인바디": ["코스닥", "공시", "실적", "주가", "수주", "계약", "식약처", "FDA", "의료기기", "웨어러블", "체성분 분석기"],
}
...
def merge_cross_company_duplicates(items):
    ...
    나눈 뒤에 호출하면(예전 버그) 서로 다른 카테고리 회사가 함께 언급된 기사(예: 미용의
    "엘앤씨바이오"와 치료제의 "시지바이오"가 같이 나오는 기사)가 두 카테고리 모두에
    중복으로 남는다 — 사용자가 "기사 개수가 너무 많다"고 보고해서 확인해보니 실제로 같은
    URL이 서로 다른 카테고리에 두 번씩 들어가 있었다(엘앤씨바이오 특허 기사가 미용·
    치료제 둘 다에, 유한양행 공급계약 기사가 미용·로보틱스·치료제 셋 다에
