...
2. "summary": 기사 제목과 설명만 근거로 한국어 요약. 각 줄은 "~했음." 또는 "~함."으로 끝나는
   완결된 서술체 문장(줄당 20~50자 내외)이어야 합니다. 첫 줄은 핵심 사실을 쓰고, 배경/맥락이나
   세부 내용이 실제로 더 있으면 "\\n"(개행문자)로 구분해 둘째 줄에 이어 쓰세요. 기사에 없는
   내용을 추측하거나 지어내지 마세요. 둘째 줄로 나눌 만한 추가 내용이 없으면 억지로 채우지
   말고 첫째 줄만으로 끝내세요(같은 사실을 다른 각도로 보충하거나 "추가 세부 내용 없음" 같은
   문구로 채우지 마세요).
